...
export const PopularDestinations: React.FC<PopularDestinationsProps> = ({
  ...
  // Localized texts
  const badgeText = language === 'UR' ? 'دنیا کے مشہور ترین شہر' : language === 'AR' ? 'أشهر المدن والوجهات العالمية' : 'Top Global Cities';
  const headingText = language === 'UR' ? 'دنیا بھر کے مقبول ترین مقامات' : language === 'AR' ? 'أشهر الوجهات السياحية العالمية' : 'Popular Worldwide Destinations';
  const subtitleText = language === 'UR' 
    ? 'ثقافتی عجائبات، تاریخی مقامات اور عالمی شہرت یافتہ سیاحتی مراکز کے سحر انگیز اسفار۔'
    : language === 'AR'
    ? 'اكتشف روائع التراث الإنساني، معالم المعمار الخالدة وعواصم السياحة العالمية بأفضل الأسعار.'
    : 'Immerse yourself in cultural marvels, culinary capitals, and world-renowned architectural landmarks.';

  const getCityName = (dest: PopularDestination) => {
    if (language === 'AR') {
      const arCities: Record<string, string> = {
        Dubai: 'دبي',
        Istanbul: 'إسطنبول',
        London: 'لندن',
      ...
      return arCities[dest.city] || dest.city;
    }
    if (language === 'UR') {
      const urCities: Record<string, string> = {
        Dubai: 'دبئی',
        Istanbul: 'استنبول',
        London: 'لندن',
      ...
      return urCities[dest.city] || dest.city;
    }
    return dest.city;
  };

  const getCountryName = (dest: PopularDestination) => {
    if (language === 'AR') {
      const arCountries: Record<string, string> = {
      ...
      return arCountries[dest.country] || dest.country;
    }
    if (language === 'UR') {
      const urCountries: Record<string, string> = {
      ...
      return urCountries[dest.country] || dest.country;
    }
    return dest.country;
  };

  const fromLabel = language === 'UR' ? 'شروع قیمت' : language === 'AR' ? 'تبدأ من' : 'From';
  const exploreLabel = (cityName: string) =>
    language === 'UR'
      ? `${cityName} کی پروازیں دیکھیں`
      : language === 'AR'
      ? `استكشف رحلات ${cityName}`
      : `Explore Flights to ${cityName}`;
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">
          {POPULAR_DESTINATIONS.map((dest) => {
            const localizedCity = getCityName(dest);
            const localizedCountry = getCountryName(dest);
              ...
              >
                ...
                <div className="absolute bottom-0 left-0 right-0 p-6 flex flex-col justify-end">
                  <div className="flex items-center justify-between mb-1">
                    <span className={`text-xs font-bold text-gray-300 uppercase tracking-widest flex items-center gap-1 ${fontClass}`}>
                      <MapPin className="w-3 h-3 text-[#E53935]" />
                      {localizedCountry}
                    </span>
                    <span className={`text-xs font-bold text-white bg-[#E53935] px-2 py-0.5 rounded ${fontClass}`}>
                      {fromLabel} {formatCurrency(dest.startingPriceUSD, currency)}
                    </span>
                  </div>

                  <h3 className={`text-2xl font-black text-white font-heading tracking-tight mb-2 ${fontClass}`}>
                    {localizedCity} ({dest.airportCode})
                  </h3>

                  <p className={`text-xs text-gray-200 line-clamp-2 mb-4 leading-relaxed ${fontClass}`}>
                    {dest.description}
                  </p>
                  ...
                  <button
                    type="button"
                    className={`w-full py-2.5 rounded-xl bg-white/15 hover:bg-white text-white hover:text-[#071A3D] text-xs font-bold transition-all duration-200 flex items-center justify-center gap-2 backdrop-blur-md ${fontClass}`}
                  >
                    <span>{exploreLabel(localizedCity)}</span>
